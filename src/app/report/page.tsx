...
import { useRouter } from 'next/navigation';
import { toast } from 'react-hot-toast'
import { analyzeWasteImage, WasteVerificationResult, createImageHash } from '@/utils/db/gemini'
...
export default function ReportPage() {
  ...
  const handleManualInput = () => {
    if (verificationStatus === 'failure') {
      ...
      const manualResult: WasteVerificationResult = {
        wasteType: 'General Waste',
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      // Use the centralized image analysis function in report mode
      ...
      // Update state with the standardized result
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSubmitting(true);
    try {
      // Create a metadata object to store with the report
      const metadata = {
